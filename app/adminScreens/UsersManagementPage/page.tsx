...
export default function UserManagementPage() {
  ...
  if (authLoading) return <div className="p-10 text-center">טוען נתונים...</div>;

  return (
    <main className="min-h-screen bg-gray-50 p-6" dir="rtl">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">ניהול משתמשים וקבוצות</h1>
        ...
        <section className="mb-10">
          <h2
            className={`text-xl font-bold mb-4 border-b-2 pb-2 ${
              activeTab === "admins" ? "text-purple-600 border-purple-200" :
              activeTab === "groups" ? "text-orange-600 border-orange-200" :
          ...
          {isGroupsTab ? (
            groupRequests.length > 0 ? (
              <div className="bg-white rounded-lg shadow border-l-4 border-orange-400 divide-y">
                {groupRequests.map((req) => (
                  <div key={req.id} className="p-4 flex flex-col md:flex-row justify-between items-center hover:bg-orange-50 transition-colors">
                    <div className="mb-2 md:mb-0">
                      <div className="font-bold text-lg text-gray-800">
                        {req.activities?.title || "פעילות לא ידועה"}
                      </div>
                      <div className="text-gray-600">👤 {req.users?.full_name}</div>
                      <div className="text-gray-500 text-sm">📅 {new Date(req.created_at).toLocaleDateString("he-IL")}</div>
                    </div>
            ...
            pendingUsers.length > 0 ? (
              <div className="bg-white rounded-lg shadow border-l-4 border-orange-400 divide-y">
                {pendingUsers.map((u) => (
                  <div key={u.id} className="p-4 flex flex-col md:flex-row justify-between items-center hover:bg-orange-50 transition-colors">
                    <div className="mb-2 md:mb-0">
                      <div className="font-bold text-lg flex items-center gap-2">
                        {u.full_name}
                        {isShowingAdmins && <span className="bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded">ADMIN REQUEST</span>}
                      </div>
                      <div className="text-gray-600">{u.email}</div>
                      <div className="text-gray-500 text-sm">{u.phone}</div>
                    </div>
                    <div className="flex gap-3 w-full md:w-auto">
                      <Link href={`/adminScreens/UserQuizPage?id=${u.id}`} className="flex-1 md:flex-none text-center bg-gray-100 text-gray-700 px-4 py-2 rounded font-medium hover:bg-gray-200">
                        📄 שאלון
                      </Link>
                      <button onClick={() => handleApproveUser(u.id)} disabled={!!processingId} className="flex-1 md:flex-none bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded font-bold shadow-sm">
                        ✓ אשר
                      </button>
                      <button onClick={() => handleDeleteUser(u.id)} disabled={!!processingId} className="flex-1 md:flex-none bg-red-100 hover:bg-red-200 text-red-600 px-4 py-2 rounded font-bold">
                        ✗ דחה
                      </button>
                    </div>
                  </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">מידע נוסף</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {approvedUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap"><div className="font-bold text-gray-900">{u.full_name}</div></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><div>{u.email}</div><div>{u.phone}</div></td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <Link href={`/adminScreens/UserQuizPage?id=${u.id}`} className="text-blue-600 hover:text-blue-800 font-medium hover:underline">
                          צפה בנתוני שאלון
                        </Link>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <button onClick={() => handleDeleteUser(u.id)} disabled={!!processingId} className="text-red-600 hover:text-red-900 font-bold bg-red-50 px-3 py-1 rounded">
                          מחק
                        </button>
                      </td>
                    </tr>
                  ...
                  {approvedUsers.length === 0 && (
                    <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-500">לא נמצאו תוצאות.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        {!isGroupsTab && (
          <Link href="/adminScreens/AddAdminPage" className="text-blue-600 hover:text-blue-800 font-medium hover:underline block mt-4 text-center md:text-right">
            הוספת אדמין
          </Link>
        )}
      </div>
